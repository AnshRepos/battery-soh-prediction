...
my $hT;                                                              # !!! Hash Zwischenspeicher
	...
	$value= $ref->{$key};
	      ...
	      if(!defined($value)) {
	         unless(exists($self->{opt}->{suppressempty}) and !defined($self->{opt}->{suppressempty}) ) {
         	    carp 'Use of uninitialized value';
	         }
      	  $value = {};
	      }
	      if(ref($value)  or  $self->{opt}->{noattr}) {
	            push @nested,    $self->value_to_xml($value, $key, $encoded, "$indent  ");
	      }
	      else {
...
$hT->{$key} = $value;                                                   # !!! Hash Zwischenspeicher
#	            push @result, ' ', $key, '="', $value , '"';
	         }
         }
      }
      my $k;                                                            # !!! Hash Zwischenspeicher
      foreach $k (sort keys %$hT )# !!! Hash Zwischenspeicher
      {                                                                 # !!! Hash Zwischenspeicher
         push @result, ' ', $k, '="', $hT->{$k} , '"';                  # !!! Hash Zwischenspeicher
      }                                                                 # !!! Hash Zwischenspeicher
    }
    else {
      $text_content = '';
